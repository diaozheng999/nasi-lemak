...
export function Component(): unknown {
  ...
  const { Unique, Disposable } = jest.requireActual("nasi");
  ...
  > extends React.Component<TProps, TState> implements IDescribable {
     ...
     * ```
     * This will allow us to still use the legacy `Theme.setTheme()`-style
     * tests.
    ...
    protected reducer(__: TState, ___: TAction): any {
      return null;
    }
